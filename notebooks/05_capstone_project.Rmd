...
ab_full <- ab_long |>
  ...
  inner_join(select(meta_clean, sample_id, nationality, bmi_group, age),
             by = "sample_id")     # inner_join keeps only samples with metadata
glimpse(ab_full)
```
</details>
...
alpha <- ab_full |>
  ...
  summarise(
    ...
    shannon  = diversity(abundance, index = "shannon"),  # vegan
    .groups  = "drop")
...
```{r}
ggplot(alpha, aes(bmi_group, shannon, fill = bmi_group)) +
...
# Build a sample x genus matrix (use a manageable subset for speed in class).
set.seed(1)
sub_ids <- meta_clean |>
  filter(nationality %in% c("Scandinavia","US","SouthEurope","CentralEurope")) |>
  group_by(nationality) |> slice_sample(n = 40) |> pull(sample_id)

mat <- abund |>
  filter(sample_id %in% sub_ids) |>
  tibble::column_to_rownames("sample_id") |>
  as.matrix()
mat <- mat[rowSums(mat) > 0, ]              # drop any empty samples
...
ggplot(scores_df, aes(NMDS1, NMDS2, colour = nationality)) +
  geom_point(size = 2, alpha = 0.8) +
  stat_ellipse(level = 0.7) +               # a 70% confidence ellipse per region
...
```{r}
write_csv(alpha,       "capstone_alpha_diversity.csv")
write_csv(composition, "capstone_phylum_by_region.csv")
